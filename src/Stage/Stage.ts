...
export class Stage {
  ...
  constructor({ container, size, perspective, getTime }: { container: HTMLElement; size?: [number, number]; perspective?: number; getTime: () => number; }) {
    ...
    this.el.classList.add("m7-danmaku-stage")
    ...
    this.stylesheet.innerText += ".m7-danmaku-stage.paused .m7-danmaku-item { animation-play-state: paused }"
    this.stylesheet.innerText += ".m7-danmaku-item.m7-danmaku-invisible { visibility: hidden }"
    this.stylesheet.innerText += ".m7-danmaku-item-content { -webkit-text-size-adjust: none; text-size-adjust: none}"
  ...
  public add(dan: DanmakuData) {
    ...
    if(dan.end > this.getTime()) {
      ...
      // 添加开始前隐藏机制
      danmakuElement.classList.add("m7-danmaku-invisible")
      danmakuElement.addEventListener("animationstart", ()=>{
        danmakuElement.classList.remove("m7-danmaku-invisible");
      }, {once: true})
      // 添加结束后销毁机制
  ...
  public remove_rid(rid: string) {
    let element = this.el.querySelector(`.m7-danmaku-item-${rid}`)
    element && this.el.removeChild(element)
  }
  /** 清屏 */
  public clear() {
